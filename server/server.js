/*
 * Direct Alerte Server
 * node "server/server.js"
 * DO NOT require("./server") in other modules, it likely creates circular dependency!
 */
console.log("Welcome to Direct Alerte");
...
// Exit Direct Alerte immediately if the Node.js version is banned
...
log.info("server", "Direct Alerte Version: " + checkVersion.version);
        ...
        socket.on("setup", async (username, password, callback) => {
            try {
                ...
                if ((await R.knex("user").count("id as count").first()).count !== 0) {
                    throw new Error("Direct Alerte has been initialized. If you want to run setup again, please delete the database.");
                }
...
async function initDatabase(testMode = false) {
    ...
    // If there is no record in user table, it is a new Direct Alerte instance, need to setup
    if ((await R.knex("user").count("id as count").first()).count === 0) {
